          ...
          $$(go.Node, "Auto", 
            {
              click: (e, node) => {
                 ...
                 var data = {}
                 this.currentkey = node.data.realkey;
                 ...
                 this.changewordtable(); // 여기서 @를 모두 적정하게 바꿈
...
*/
  ...
  nodedatasetting_func(first){
	  ...
	  var arr = []
	   for(var i in first){
		   ...
		   this.whilefunc(null, first[i].processname1, 0, 0, shape);
	   }
  ...
  whilefunc(parentname, childname, current_x, current_y, shape){
      ...
      var obj = {key : text, fig: "Rectangle", wid: text.length * 15, 
                 hei: 50, angle: 0, color: "white", loc: x + " " + y,
      ...
      if(turn.superpro){
    	 ...
    	 for(var k in turn.superpro){
             this.whilefunc(text, turn.superpro[k], current_x, current_y);  
             current_x++;
    	 }
  ...
  nodedatasetting_graph = (first) => {
   ...
   while(count > 0 && co < 10){
    ...
    for(var i in arr){
      ...
      var obj = {key : turn.processname1, fig: "Rectangle", wid: turn.processname1.length * 15, 
                 hei: 50, angle: shape[i].angle, color: "white", loc: x + " " + y,
         ...
         // 위치결정
	      ...
	      for(var i1 = 1; i1 < 5; i1++){
	    	  if(turn["processname" + i1]){
	    		  ...
	    		  var obj = {group: arr[i], key : turn["processname" + i1],  fig: "Rectangle", wid: this.textwid,  //turn.name.length * 15
	 	                 hei: 50, angle: shape[i].angle, color: "white", loc: x + " " + y,
                ...
                for(var num = 1; num <=5; num++){
                 	if(parameter1 + num in data[i]){
                		if(data[i][parameter1 + num] != ""){
                           // tablearr[r][0].innerText = data[i][parameter1 + num];
