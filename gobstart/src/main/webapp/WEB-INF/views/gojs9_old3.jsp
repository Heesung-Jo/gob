 ...
 <html>
    ...
    <body>
      ...
      <div style="width: 1300px; display: flex; justify-content: space-between">
         <div id="myDiagramDiv" style="border: solid 1px blue; width:1000px; height:400px"></div>
    ...
    while(count > 0 && co < 10){
     ...
     for(var i in arr){
      ...
      // 다음 순환을 위한 세팅하기
       ...
       var tempteam = {}
       for(var i in next){
          if(this.totalarr[next[i]].teamname in tempteam){
             tempteam[this.totalarr[next[i]].teamname].push(next[i]);
          }else{
             tempteam[this.totalarr[next[i]].teamname] = [next[i]];
          }
  ...
  nodedatasetting_graph2 = (first) => {
	   ...
	   while(count > 0 && co < 10){
	    ...
	    for(var i in arr){
	      ...
	      var obj = {group: turn.teamname, key : turn.processname1,  fig: "Rectangle", wid: this.textwid,  //turn.name.length * 15
	                 hei: 50, angle: shape[i].angle, color: "white", loc: x + " " + y,
	                 parent: turn.parentnodedata, realkey: turn.processname1, stretch: go.GraphObject.Vertical}
	      
	      var obj2 = [];
	      for(var j in turn.next){
	         var temp = {from: turn.processname1,to: turn.childnodedata[j]}   
	         obj2.push(temp);     
	      }
...
// tag 관련
    ...
    start = (data) => {
        ...
        for(var i in this.totalarr){
        	...
        	if(this.totalarr[i].parentnodedata.length > 0){
        	  if(this.totalarr[i].parentnodedata[0] == "start"){
